...
def home_page(l):
 ...
 why=f'<section class="home-why section" id="philosophy"><div class="wrap"><span class="eyebrow">WHY AP</span><h2>{e(h["why"])}</h2>'+(f'<p class="why-lead">{e(h["whyLead"])}</p>' if h.get("whyLead") else '')+f'<figure class="pic why-pic"><picture><source media="(max-width:800px)" srcset="/media/gfx/why_ap_m{"_en" if l=="en" else ""}.jpg"><img src="/media/gfx/why_ap{"_en" if l=="en" else ""}.jpg" alt="{e(h.get("whyAlt",""),quote=True)}" loading="lazy"></picture></figure>'+'<div class="principles">'+''.join(f'<article><h3>{n}</h3><p>{e(h["principles"][i])}</p></article>' for i,n in enumerate(h.get('principleNames',['DATA','EXPERTISE','PROCESS','TECHNOLOGY'])))+'</div></div></section>'
