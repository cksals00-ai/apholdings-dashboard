...
def home_page(l):
 ...
 featured=f'<section class="home-engine home-featured" id="featured"><div class="wrap"><span class="eyebrow">AP ENGINE / DATA + EXPERTISE + PROCESS + TECHNOLOGY</span><h2>{e(h["featured"])}</h2><p class="engine-intro">{e(h["engineIntro"])}</p>{engine_steps}<p class="note">{e(h["engineNote"])}</p></div></section>'
 ...
 images={'safe':'/media/safelist_frame.jpg','light':'/img/lightlist_hero.jpg','travel':'/media/inbound_frame.jpg','commerce':'/media/shop/01_dalba.jpg','craft':'/media/craft_frame.jpg','cubs':'/media/cubs/screens.jpg','lastwave':'/media/games/lw_keyart.jpg'}
 def compact(pid):
  p=prod(l,pid)
 ...
 for pid in ['safe','light','revenue','travel']:
  ...
  decision_cards+=f'<article class="engine-product"><a class="engine-product-visual" href="{ph(l,pid)}">{visual}</a><div class="engine-product-copy"><span class="eyebrow">{e(example[0])}</span><h4>{e(p["name"])}</h4><p>{e(h["short"][pid])}</p><dl><div><dt>{e(h["input"])}</dt><dd>{e(example[1])}</dd></div><div><dt>{e(h["output"])}</dt><dd>{e(example[2])}</dd></div></dl>{note}{link(ph(l,pid),detail)}</div></article>'
 ...
 for i,(gid,title,tag,ids) in enumerate(GROUPS):
  contents='<div class="engine-product-grid">'+decision_cards+'</div>' if gid=='decision' else select_feature+'<div class="commerce-extras">'+lia_feature+'<div class="compact-grid">'+compact('commerce')+compact('craft')+'</div></div>' if gid=='commerce' else rgrg+'<div class="compact-grid games-grid">'+compact('cubs')+compact('lastwave')+'</div>'
