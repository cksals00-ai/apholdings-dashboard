...
def select(l):
 p=prod(l,'select')
 return f'<article class="select-feature">{status(l,p["status"])}<h3>AP SELECT</h3><h4>A store designed<br>for choosing.</h4><p class="select-kr">{c(l,"selectKo")}</p><p class="desc">{e(p["desc"])}</p><div class="channels"><div><b>OFFLINE</b><span>Experience + Decision</span></div><div><b>ONLINE</b><span>Transaction + Fulfillment</span></div></div>{flow(p["flow"])}<div class="actions">{button(ph(l,"select"),c(l,"learnMore"))}</div></article>'
def lia(l):
...
def home_page(l):
 hero=f'<section class="hero"><div class="wrap"><span class="eyebrow">AP HOLDINGS / DATA. EXPERTISE. PROCESS. TECHNOLOGY.</span><div class="hero-grid"><h1>We turn<br>complexity<br>into <span>systems.</span></h1><div class="hero-aside"><p class="brand-line">Data. Expertise.<br>Process. Technology.</p><p class="lead">{c(l,"hero")}</p><div class="actions">{button("#portfolio",c(l,"explore"),True)}{link("#about",c(l,"aboutCta"))}</div></div></div><div class="global-line"><strong>Built in Korea. Designed for the world.</strong><span>Independent thinking. Repeatable systems.</span></div></div></section>'
