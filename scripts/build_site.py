...
def footer(l):
 ...
 cols+=col('Play & Learn',[(prod(l,x)['name'],ph(l,x)) for x in ['rgrg','cubs','lastwave']]+[('AP Games','https://games.apholdings.kr/'+l+'/')])
...
def lab(l,full=False):
 cards=''.join(f'<article class="lab-card"><span class="lab-icon">{"".join(w[0] for w in n.replace("The ","").split()[:2]).upper()}</span><h3>{e(n)}</h3><p>{e(d)}</p>{link("https://apps.apple.com/kr/app/id"+app,"App Store ↗","text-link",True)}</article>' for n,d,app,_ in DATA[l]['lab'][:None if full else 3])
...
def home_page(l):
 ...
 images={'safe':'/media/safelist_frame.jpg','travel':'/media/gfx/travel_card.jpg','commerce':'/media/shop/01_dalba.jpg','craft':'/media/craft_frame.jpg','cubs':'/media/cubs/screens.jpg','lastwave':'/media/games/lw_pov_lotte.jpg'}
 def compact(pid):
  p=prod(l,pid)
 ...
 if d:
  ...
  decision_showcase=f'<article class="showcase-select">{safe_visual}{safe_text}</article><div class="commerce-extras">{light_feature}<div class="compact-grid">'+compact2('revenue',f'/media/gfx/revenue_card{"_en" if l=="en" else ""}.jpg','AP Revenue — 07:00 오늘의 판단, 취소 노출',d['revenue'])+compact2('travel',f'/media/gfx/travel_card{"_en" if l=="en" else ""}.jpg','AP Travel — 일본→서울 · 30대 커플 · 11월 첫째 주',d['travel'])+'</div></div>'
 else:
 ...
 for i,(gid,title,tag,ids) in enumerate(GROUPS):
  ...
  games_link=f'<p class="group-site"><a href="https://games.apholdings.kr/{l}/">'+('AP Games 공식 사이트 — LAST WAVE' if l=='ko' else 'AP Games official site — LAST WAVE')+' <span aria-hidden="true">↗</span></a></p>'
  groups+=f'<section class="portfolio-group" id="{gid}" aria-label="{e(title)}"><div class="group-head"><span>0{i+1}</span><h3>{e(h["groups"][i])}</h3></div><p class="group-desc">{e(h["groupDesc"][i])}</p>{games_link if gid=="games" else ""}{contents}</section>'
 ...
 why=f'<section class="home-why section" id="philosophy"><div class="wrap"><span class="eyebrow">WHY AP</span><h2>{e(h["why"])}</h2>'+(f'<p class="why-lead">{e(h["whyLead"])}</p>' if h.get("whyLead") else '')+('<ol class="why-flow">'+''.join(f'<li><span class="wf-step">{i+1:02d} · {e(x["step"])}</span><p class="wf-big">{e(x["big"])}<small>{e(x["unit"])}</small></p><h3>{e(x["title"])}</h3><p class="wf-sub">{e(x["sub"])}</p></li>' for i,x in enumerate(h["whyFacts"]))+'</ol>' if h.get("whyFacts") else '')+'</div></section>'
...
def ir_page(l):
 ...
 secs=DATA[l]['copy'].get('irSections')
...
def product_page(l,p):
 ...
 if pid=='lastwave' and not p.get('hideGeneric'):
  body+='<h2 style="margin-top:45px">Characters &amp; worlds</h2><div class="grid-3" style="margin-top:24px">'+''.join(f'<figure style="margin:0"><img loading="lazy" src="/media/games/{src}" alt="{label}" width="500" height="500"><figcaption class="note">{label}</figcaption></figure>' for src,label in [('lw_hero_pixel.png','PIXEL'),('lw_hero_alfred.png','ALFRED'),('lw_hero_line.png','LINE'),('lw_hero_bluenewbie.png','BLUE NEWBIE'),('lw_hero_iris.png','IRIS'),('lw_pov_gwanghwamun.jpg','World concept')])+'</div>'
 if pid=='commerce' and not p.get('hideGeneric'):
