...
def home_page(l):
 ...
 for pid in ['safe','light','revenue','travel']:
  ...
  visual=f'<img src="{images[pid]}" alt="{e(p["name"])} — product concept" width="800" height="450" loading="lazy">' if pid in images else '<div class="revenue-example"><span>AP LIGHT</span><strong>'+('한 봉지 465kcal' if l=='ko' else 'One bag: 465 kcal')+'</strong><div>'+('포장 뒷면은 100g당 517kcal' if l=='ko' else 'Back label says 517 kcal / 100 g')+'</div></div>' if pid=='light' else '<div class="revenue-example"><span>AP REVENUE</span><strong>'+('어디에 집중할 것인가?' if l=='ko' else 'Where to focus next?')+'</strong><div>'+('고객 · 시장 · 채널' if l=='ko' else 'Accounts · Markets · Channels')+'</div></div>'
