...
def footer(l):
 ...
 cols+=col('Play & Learn',[(prod(l,x)['name'],ph(l,x)) for x in ['rgrg','cubs','lastwave']]+[('AP Games','http://games.apholdings.kr/'+l+'/')])
...
def home_page(l):
 ...
 for i,(gid,title,tag,ids) in enumerate(GROUPS):
  ...
  games_link=f'<p class="group-site"><a href="http://games.apholdings.kr/{l}/">'+('AP Games 공식 사이트 — LAST WAVE' if l=='ko' else 'AP Games official site — LAST WAVE')+' <span aria-hidden="true">↗</span></a></p>'
